    ...
    try {
        ...
        tbody.innerHTML = users.map(user => `
            <tr onclick="viewUser('${user.id}')" style="cursor: pointer;">
                <td>${user.userId || user.uid || user.id}</td>
                <td>${user.email || 'N/A'}</td>
...
async function viewUser(userId) {
    try {
        ...
        document.getElementById('userDetailTitle').textContent = `User: ${user.userId || user.id}`;
        document.getElementById('detailUserId').textContent = user.userId || user.id;
        ...
        await loadUserReceivedHistory(user.userId || user.id);
        
        // Load deposit history
        await loadUserDepositHistory(user.userId || user.id);
...
async function loadUserReceivedHistory(userId) {
    try {
        ...
        tbody.innerHTML = received.length > 0 
            ...
            `).join('')
            : '<tr><td colspan="2" class="empty-state">No received entries</td></tr>';
    } catch (error) {
    ...
    container.innerHTML = sortedActivities.map(activity => `
        <div class="activity-item">
            ...
            <div class="activity-content">
                ...
                <div class="activity-meta">User: ${activity.userId} • ${new Date(activity.timestamp).toLocaleString()}</div>
            </div>
